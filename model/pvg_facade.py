  ...
  def init_weights(self, init_type='normal', gain=0.02):
    ...
    def init_func(m):
      ...
      if classname.find('InstanceNorm2d') != -1:
        ...
        '''
        Gain is a proportional value that shows the relationship between 
        the magnitude of the input to the magnitude of the output signal at steady state. 
        Many systems contain a method by which the gain can be altered, 
...
class InpaintGenerator(BaseNetwork):
  def __init__(self, init_weights=True):
    ...
    self.dw_conv01 = nn.Sequential(
      nn.Conv2d(4, cnum*2, kernel_size=3, stride=2, padding=1),
      nn.LeakyReLU(0.2, inplace=True))

    self.dw_conv02 = nn.Sequential(
      nn.Conv2d(cnum*2, cnum*2*2, kernel_size=3, stride=2, padding=1),
      nn.LeakyReLU(0.2, inplace=True))

    self.dw_conv03 = nn.Sequential(
      nn.Conv2d(cnum*2*2, cnum*4*2, kernel_size=3, stride=2, padding=1),
      nn.LeakyReLU(0.2, inplace=True))

    self.dw_conv04 = nn.Sequential(
      nn.Conv2d(cnum*4*2, cnum*8*2, kernel_size=3, stride=2, padding=1),
      nn.LeakyReLU(0.2, inplace=True))

    self.dw_conv05 = nn.Sequential(
      nn.Conv2d(cnum*8*2, cnum*16*2, kernel_size=3, stride=2, padding=1),
      nn.LeakyReLU(0.2, inplace=True))

    self.dw_conv06 = nn.Sequential(
      nn.Conv2d(cnum*16*2, cnum*16*2, kernel_size=3, stride=2, padding=1),
  ...
  def forward(self, img, mask):
    ...
    # sampling
    mu6, sigma6 = x6.chunk(2, dim=1)
    x6 = mu6 + sigma6 * torch.randn_like(sigma6)
    mu5, sigma5 = x5.chunk(2, dim=1)
    x5 = mu5 + sigma5 * torch.randn_like(sigma5)
    mu4, sigma4 = x4.chunk(2, dim=1)
    x4 = mu4 + sigma4* torch.randn_like(sigma4)
    mu3, sigma3 = x3.chunk(2, dim=1)
    x3 = mu3 + sigma3 * torch.randn_like(sigma3)
    mu2, sigma2 = x2.chunk(2, dim=1)
    x2 = mu2 + sigma2 * torch.randn_like(sigma2)
    mu1, sigma1 = x1.chunk(2, dim=1)
    x1 = mu1 + sigma1 * torch.randn_like(sigma1)




    get_size=lambda x:x.size(0)*x.size(1)*x.size(2)*x.size(3)
    get_kld=lambda size,mu,sigma:0.5*torch.sum(torch.pow(mu,2)+torch.pow(sigma,2)-torch.log(1e-8 + torch.pow(sigma,2))-1)/(size*batchsize)
    ...
    kld6 = get_kld(x6_size, mu6, sigma6)
    kld5 = get_kld(x5_size, mu5, sigma5)
    kld4 = get_kld(x4_size, mu4, sigma4)
    kld3 = get_kld(x3_size, mu3, sigma3)
    kld2 = get_kld(x2_size, mu2, sigma2)
    kld1 = get_kld(x1_size, mu1, sigma1)
    ...
    return  pyramid_imgs, output, sum(kld)
...
class AtnConv(nn.Module):
  ...
  def forward(self, x1, x2, mask=None):
    ...
    m = m.contiguous().view(x2s[0], -1, 1, self.ksize, self.ksize)
    ...
    for xi, wi, raw_wi, mi in zip(f_groups, w_groups, raw_w_groups, mm_groups):
      ...
      xi : separated tensor along batch dimension of front; [1, C, H, W]
      wi : separated patch tensor along batch dimension of front; [1, HW, C, K, K]
      raw_wi : separated patch tensor along batch dimension of back; [1, HW, C, K, K]
      mi: separated patch tensor along batch dimension of front mask 
      '''
