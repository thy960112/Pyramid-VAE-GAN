...
class InpaintGenerator(BaseNetwork):
  def __init__(self, init_weights=True):#1046
    ...
    self.dw_conv07 = nn.Sequential(
      nn.Conv2d(cnum*8, cnum*16, kernel_size=3, stride=1, padding=1),
      nn.ReLU(inplace=True))
  ...
  def forward(self, img, mask):
    x = img
    # encoder 
    get_pic=lambda x:x.view(-1,x.size(2),x.size(3))[:10].cpu().detach().numpy()
    ...
    x6_sample = mu6 + sigma6 * torch.randn_like(sigma6)
    ...
    x6_size = get_size(x6_sample)
    kld6=get_kld(x6_size,mu6,sigma6)
    
    #new x6
    x6_new = self.dw_conv07(x6_sample)
    loss_x6=self.l2_loss(x6_new,x6)
    ...
    return  pyramid_imgs, output,kld6 + loss_x6
